<?php
// =====================================================
// admin/quanly_baiviet.php — Quản lý bài viết
// ?action=list  → Danh sách bài viết (mặc định)
// ?action=them  → Form thêm bài mới
// ?action=sua&id=5 → Form sửa bài viết
// ?action=xoa&id=5 → Xóa bài viết
// =====================================================
session_start();
...
// ══════════════════════════════════════════════════════
// XỬ LÝ LƯU BÀI VIẾT (Thêm mới + Sửa)
// ══════════════════════════════════════════════════════
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Xử lý upload ảnh
    $thumbnail = $_POST['thumbnail_cu'] ?? ''; // giữ ảnh cũ
    ...
    if (empty($errors)) {
        if ($post_id > 0) {
            ...
            // Fix bind_param không có khoảng trắng
            $stmt = $conn->prepare("
                INSERT INTO posts (title, excerpt, content, thumbnail, category_id, author, status)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ");
...
// ── Nếu POST có lỗi, giữ lại dữ liệu đã nhập ──
...
// Hàm tạo URL giữ nguyên bộ lọc khi phân trang
...
// Bôi đậm/nghiêng text trong textarea
